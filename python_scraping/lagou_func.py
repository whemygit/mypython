...
# headers=file2dit('header.txt')


def html_js_parse(html):
    ...
    for pos_inf in result:
        ...
        search_res=[]
        for key in key_list:
            search_res.append(key+'='+pos_inf[key])
        pos_list.append(search_res)
    print pos_list
    print len(pos_list)
    return pos_list

def list2file(pos_list):
    with open('position.txt','w') as fw:
        line=''
        for inf in pos_list:
            line=line+','.join(inf)+'\n'
        fw.write(line)
